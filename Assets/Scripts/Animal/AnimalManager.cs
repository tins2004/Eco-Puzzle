...
public class AnimalManager : MonoBehaviour
{
    ...
    public void SelectAnimal(AnimalData animal)
    {
        if (selectedAnimal != AnimalType.A00_Null)
        {
            ...
            ResetSelection();
            levelManager.audioManager.PlaySFXFailClick();
            return;
        }

        levelManager.audioManager.PlaySFXAnimalSound(animal.type);
    ...
    public void ShowRegionInfo(TileType tileType, List<Region> inappropriateRegions)
    {
        ...
        for (int i = 0; i < regions.Count; i++)
        {
            ...
            // Lấy danh sách thú duy nhất (loại bỏ trùng lặp và Null)
            var uniqueAnimals = animalsInRegion
                .Where(a => a != AnimalType.A00_Null)
                .Distinct()
                .ToList();

            // Nếu chỉ có 1 loại thú thì scale lớn hơn
            float scale = uniqueAnimals.Count == 1 ? 2f : 1f;

            // Hiển thị icon cho từng loại thú duy nhất
            for (int j = 0; j < uniqueAnimals.Count && j < icons.Length; j++)
            {
                var iconData = animals.Find(a => a.type == uniqueAnimals[j]);
                if (iconData != null)
                {
                    icons[j].sprite = iconData.icon;
                    icons[j].gameObject.SetActive(true);
                    icons[j].transform.localScale = Vector3.one * scale;
                }
            }
    ...
    private void SelectRegion(Region region)
    {
        ...
        // {
        //     Debug.LogWarning("Region này đã đủ 4 động vật, không thể thêm!");
        //     return;
        // }
    ...
    private void SpawnAnimalOnRegion(Region region, AnimalType animalType)
    {
        ...
        var marker = iconObj.GetComponent<AnimalIconMarker>();
        ...
        // Đánh dấu nhiệm vụ
        levelManager.missionManager.CollectAnimal(selectedAnimal, iconObj.transform.position);
        UpdateAnimalIcons();
    }
