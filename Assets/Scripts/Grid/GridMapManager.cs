...
public class GridMapManager : MonoBehaviour
{
    ...
    private void Generate()
    {
        ...
        transform.DOKill(); // dừng tất cả các tween đang chạy
        ...
        seq.AppendInterval(1f); // delay trước khi hiện UI - đợi Loading
        ...
        for (int x = 0; x < mapData.height; x++) // X = hàng
        {
            for (int y = 0; y < mapData.width; y++) // Y = cột
            {
                ...
                if (prefab != null)
                {
                    ...
                    // Hiệu ứng mọc lên bằng DOTween
                    Transform t = tile.transform;
                    t.localScale = Vector3.zero;

                    // thêm tween vào sequence
                    seq.AppendInterval(delayStepGenerateTile);
                    seq.Append(t.DOScale(Vector3.one, 0.2f).SetEase(Ease.OutBack));
                    seq.JoinCallback(() => audioManager.PlaySFXPop());
                }
            }
        }

        // Hiện UI sau khi tạo xong
        seq.AppendCallback(() =>
        {
            if (levelManager != null && levelManager.UICanvas != null)
            {
                levelManager.UICanvas.DOFade(1, 0.3f);
            }

            levelManager.uiManager.isReady = true;
        });
    }
